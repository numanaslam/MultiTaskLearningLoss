...
if exist(cxrDir, 'dir')
    ...
    fprintf('  Evaluating on Out-of-Distribution (Full CXR) data with preprocessing ensemble...\n');
...
function [results, predictions, uncertainty_stats] = evaluate_dataset_with_preprocessing_ensemble(...
    net, imds, classes, useGPU, base_preprocessing, refHist)
...
while hasdata(augDS)
    ...
    for m = 1:numel(preprocessing_methods)
        ...
        dlX = dlarray(single(imgs_processed), 'SSCB');
...
% Note: All other helper functions (load_data_and_network, createStratifiedKFold,
% calculate_mean_metrics, plot_kfold_training_curves, evaluateWithSegmentation,
% computeSegmentationMetrics, improved_hausdorff_distance, student_cam_one,
% compute_validation_accuracy_simple, evaluate_model_quick, compute_focal_loss,
% tversky_coefficient_dlarray, dice_coefficient_dlarray, iou_coefficient_dlarray,
% student_cam_one_dlarray, compute_validation_loss_with_config_preprocessed,
% preprocessMiniBatchWithPreprocessing, apply_preprocessing_batch) should be
% copied unchanged from your original final_1.m file.