...
VALIDATION_MODE = false;  % Set to false for full training
...
config.imageAugmenter = imageDataAugmenter( ...
    ...
    'RandRotation', [-30 30], ...
    'RandScale', [0.3 1.2]); % increased from [0.9 1.1]
...
fprintf('  Class weight multiplier: 2x for PTB\n');
...
if exist(cxrDir, 'dir')
    ...
    fprintf('\n  === OOD EVALUATION RESULTS (WITH ENSEMBLE + TTA) ===\n');
    fprintf('  Preprocessing: %s + ensemble + TTA\n', preprocessing_method_param);
    ...
    fprintf('\n  OOD (Full CXR) Performance (Ensemble + TTA):\n');
    fprintf('    Accuracy: %.3f\n', resultsOOD.accuracy);
...
for fold = 1:config.k_folds
    ...
    if numel(classes) == 2
        classWeights = [baseWeights(1), baseWeights(2) * 2];
    else
            ...
            if ~isempty(realMask) && any(realMask(:))
                ...
                if isfield(loss_config, 'adaptive_scaling') && loss_config.adaptive_scaling
                    scale_factor = min(loss_config.max_anatomical_scale, ...
                        max(100, epoch * 20));  % Linear ramp: 100→1000 over 50 epochs
                else
...
% Evaluate with multiple preprocessing methods, TTA (Flip), and ensemble predictions
...
while hasdata(augDS)
    ...
    % === IMPROVEMENT: Preprocessing ensemble + TTA Loop ===
    probs_batch = zeros(size(imgs, 4), numel(classes));
    
    % If OOD, we might want to extract ROI first (optional, depends on your pipeline)
    % For this example, we assume preprocessing is applied to the input images directly.
    % If you have ROI extraction logic, apply it here before the method loop.
    base_imgs = imgs; 
    if is_ood_evaluation
        % Extract ROI logic if needed (simplified placeholder)
        % base_imgs = extract_roi_batch(imgs); 
    end

    for m = 1:numel(preprocessing_methods)
        method = preprocessing_methods{m};
        imgs_processed = apply_preprocessing_batch(base_imgs, method, refHist);
        
        % --- TTA: Original Image ---
        if useGPU, imgs_gpu = gpuArray(imgs_processed); else imgs_gpu = imgs_processed; end
        dlX = dlarray(single(imgs_gpu), 'SSCB');
        probs_batch = probs_batch + extractdata(predict(net, dlX))';
        
        % --- TTA: Horizontal Flip ---
        % Flip along width (Dimension 2)
        imgs_flip = flip(imgs_processed, 2);
        if useGPU, imgs_flip_gpu = gpuArray(imgs_flip); else imgs_flip_gpu = imgs_flip; end
        dlX_flip = dlarray(single(imgs_flip_gpu), 'SSCB');
        probs_batch = probs_batch + extractdata(predict(net, dlX_flip))';
    end
    
    % Average probabilities across preprocessing methods and TTA flips
    % We made 2 predictions (orig + flip) for each method.
    probs_batch = probs_batch / (numel(preprocessing_methods) * 2);
    % ================================================
...
function [X, T] = preprocessMiniBatchWithPreprocessing(dataX, dataT, preprocessing_method, refHist)
    ...
    for b = 1:B
        img = X(:, :, :, b);
        if C == 3, img_gray = rgb2gray(img); else, img_gray = img; end
        switch preprocessing_method
            ...
            case 'histmatch'
                if ~isempty(refHist), img_processed = histeq(img_gray, refHist); else, img_processed = img_gray; end
...
function [accuracy, precision, sensitivity, specificity, f1score, auc, ...
    ...
    while hasdata(mbqEval)
        ...
        if size(probs_batch, 1) == 2, Yprobs = [Yprobs; probs_batch(2,:)']; else, Yprobs = [Yprobs; probs_batch(1,:)']; end
    end
    ...
    if numel(classes)==2 && all(size(cm)==[2 2])
        ...
        [~,posIdx] = min(countcats(Ytrue)); pos = classes{posIdx};
        TP = sum(Ypred==pos & Ytrue==pos); FP = sum(Ypred==pos & Ytrue~=pos);
        FN = sum(Ypred~=pos & Ytrue==pos); TN = sum(Ypred~=pos & Ytrue~=pos);
    end
    precision = TP / (TP + FP + eps); sensitivity = TP / (TP + FN + eps); specificity = TN / (TN + FP + eps);
    f1score = 2*precision*sensitivity / (precision + sensitivity + eps);
    try [~,~,~,auc] = perfcurve(double(Ytrue == classes{2}), Yprobs, 1); catch, auc = 0.5; end
    
    numVal = numel(imdsVal.Files);
    ious = zeros(numVal, 1); dices = zeros(numVal, 1); tverskys = zeros(numVal, 1); jaccards = zeros(numVal, 1); hausdorffs = zeros(numVal, 1);
    for i = 1:numVal
        img = imread(imdsVal.Files{i});
        if size(img,3)==1, img = repmat(img,[1 1 3]); end
        img = apply_preprocessing_batch(img, config.preprocessing_method, config.refHist); % Preprocess before CAM
        img = imresize(img,[224 224]);
        try
            ...
            if size(realMask, 1) ~= 224 || size(realMask, 2) ~= 224, realMask = imresize(realMask, [224 224], 'nearest'); end
            if isempty(predCAM) || all(predCAM(:) == 0) || all(isnan(predCAM(:))), continue; end
            if size(predCAM, 1) ~= 224 || size(predCAM, 2) ~= 224, predCAM = imresize(predCAM, [224 224]); end
            cam_values = predCAM(:);
            ...
            if ~any(predMask(:)), threshold = prctile(cam_values, 25); predMask = predCAM > threshold; if ~any(predMask(:)), continue; end; end
            predMask = imopen(predMask, strel('disk', 2)); predMask = imclose(predMask, strel('disk', 3)); predMask = imfill(predMask, 'holes'); predMask = logical(predMask);
            realMask = logical(realMask);
            [ious(i), dices(i), tverskys(i), jaccards(i), hausdorffs(i)] = computeSegmentationMetrics(predMask, realMask);
        catch, end
    end
    iou = mean(ious(~isnan(ious))); dice = mean(dices(~isnan(dices))); tversky = mean(tverskys(~isnan(tverskys)));
    jaccard = mean(jaccards(~isnan(jaccards))); hausdorff = mean(hausdorffs(~isnan(hausdorffs)));
end
...
function valAcc = compute_validation_accuracy_simple(net, mbqVal, classes)
    reset(mbqVal); correct = 0; total = 0;
    while hasdata(mbqVal)
        [X, T] = next(mbqVal); Y = predict(net, X);
        Ypred = onehotdecode(extractdata(Y), classes, 1); Ytrue = onehotdecode(extractdata(T), classes, 1);
        correct = correct + sum(Ypred == Ytrue); total = total + numel(Ypred);
    end
...
function [val_acc, val_iou, val_dice] = evaluate_model_quick(net, imdsVal, classes, useGPU, ...
    ...
    try
        ...
        if iscell(precomputedGradCAM), gradCAM_subset = precomputedGradCAM(subset_idx); masks_subset = precomputedMasks(subset_idx);
        else, gradCAM_subset = precomputedGradCAM; masks_subset = precomputedMasks; end
    ...
    catch, val_acc = 0.5; val_iou = 0.0; val_dice = 0.0; end
end
