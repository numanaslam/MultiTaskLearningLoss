...
while hasdata(augDS)
    ...
    for m = 1:numel(preprocessing_methods)
        method = preprocessing_methods{m};
          ...
          % --- NEW LOGIC: Test-Time ROI Extraction ---
            if is_ood_evaluation
                % Extract center ROI to simulate lung crop on full CXR
                [H, W, ~, B] = size(imgs);
                roi_batch = zeros(H, W, 3, B, 'like', imgs); % ✅ Fixed: Initialize with 3 channels
                ...
                for b = 1:B
                    ...
                    % Extract and resize back to 224x224
                    roi_cropped = extract_lung_roi_simple(img_gray);
                    
                    % Replicate channels to match VGG16 input expectation
