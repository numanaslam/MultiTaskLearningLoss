...
%
  ...
  Performance Degradation:
    ...
    F1-Score: 6.36%
    AUC: 14.26%
    Mean Degradation: 14.93%
    Status: ✓ Good generalization (<15% degradation)
...
% Optimal hyperparameters (from Kriging optimization)
    % NOTE: Reduced lambda_cam from 4.5290 to 0.5 to balance with anatomical loss
    % GradCAM loss was overwhelming anatomical guidance (contributing >99% of total loss)
    % This reduction allows anatomical loss to have meaningful influence
    config.lambda_cam = 0.5;  % Reduced from 4.5290 to balance loss components
...
loss_config.anatomical_reward_weight = 0.5;  % NEW: Weight for rewarding attention inside lungs (0.5 = half weight of penalty)
...
fprintf('  K-Folds: %d\n', config.k_folds);
fprintf('  λ_cam: %.4f (REDUCED from 4.53 to balance with anatomical loss)\n', config.lambda_cam);
fprintf('  Reason: GradCAM was contributing >99%% of total loss, overwhelming anatomical guidance\n');
...
if loss_config.use_gradcam || loss_config.use_segmentation || ...
   loss_config.use_tversky || loss_config.use_iou || loss_config.use_anatomical_guidance
        ...
        if loss_config.use_segmentation || loss_config.use_tversky || loss_config.use_iou
            ...
            if loss_config.use_anatomical_guidance
                ...
                if ~isempty(realMask) && any(realMask(:))
                    ...
                    % reward_weight (0.5) makes reward half as strong as penalty
    ...
    %
    % Weighted contributions (with current lambda values):
    %   - Classification: ~0.01-0.1 (weight: 1.0)
    %   - GradCAM: ~500-1000 (weight: 0.5, reduced from 4.53)
    %   - Tversky: ~1.75-2.0 (weight: 2.5)
    %   - Anatomical: ~150-750 (weight: 15.0, scaled loss: 10-50)
    %
    ...
    if loss_config.use_gradcam
        ...
        % Reduced weight (0.5) to balance with anatomical loss
    ...
    if loss_config.use_anatomical_guidance
        % Anatomical guidance loss: NEW - Guides attention to lung regions
