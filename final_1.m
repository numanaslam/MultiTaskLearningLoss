...
%       'clahe' - CLAHE (Contrast Limited Adaptive Histogram Equalization)
    %       'histmatch' - Histogram matching to ROI reference (RECOMMENDED)
...
%
    ...
    %   ========================
    %   1. Focal Loss: Handles class imbalance (PTB vs Normal)
    %   2. GradCAM Loss: Ensures student model attention matches teacher GradCAM maps
    %   3. Tversky Loss: Encourages attention alignment with lung masks
    %   4. Anatomical Guidance Loss: NEW - Penalizes attention outside lungs, rewards inside
    %
    %   KEY IMPROVEMENTS (v2.3 - OPTIMIZED PTB BIAS):
    %   ==============================================
    %   1. Increased lambda_cam from 0.5 to 1.0:
    %      - Improves segmentation quality (IoU/Dice)
    %      - Better alignment with teacher GradCAM maps
    %
    %   2. Increased lambda_tversky from 2.5 to 5.0:
    %      - Stronger signal for segmentation alignment
    %      - Better IoU/Dice scores
    %
    %   3. OPTIMIZED Focal Loss Configuration (v2.3):
    %      - Focal alpha: [0.32, 0.68] (reduced from [0.30, 0.70] in v2.2)
    %      - v2.2 [0.30, 0.70] with 1.3x multiplier = 3.03x bias was too aggressive
    %      - Result: Specificity collapsed (0.255 CV, 0.039 OOD) despite high sensitivity (0.899)
    %      - New [0.32, 0.68] provides 2.13x PTB weight (reduced from 2.33x)
    %      - Class weight multiplier: 1.25x for PTB (reduced from 1.3x)
    %      - Combined effect: 2.13 × 1.25 = 2.66x PTB bias (intermediate between v2.1 and v2.2)
    %      - Goal: Balance sensitivity (0.75-0.85) and specificity (0.70-0.85)
    %      - Focal gamma: 1.5 (reduced from 2.0 to reduce over-focusing)
    %
    %   4. Improved Anatomical Loss Formulation:
    %      - Penalizes: attention outside lung regions (non-lung mask)
    %      - Rewards: attention inside lung regions (lung mask)
    %      - Formula: penalty_outside - (reward_weight * reward_inside)
    %      - Reward weight: 0.75 (increased from 0.5 to better encourage lung focus)
    %
    %   5. Anatomical Loss Scaling:
    %      - Raw anatomical loss values: [0, 1] (normalized CAM)
    %      - GradCAM loss values: ~1000-2000 (MSE)
    %      - Scale anatomical loss by 1000 to match GradCAM magnitude
    %      - Without scaling: anatomical loss <0.1% of total
    %      - With scaling: anatomical loss 20-40% of total
    %
    %   6. Increased lambda_anatomical to 15.0:
    %      - Provides strong signal for anatomical guidance
    %      - Combined with scaling, ensures meaningful contribution
    %
    %   7. Increased nCam from 16 to 32:
    %      - More samples per batch for anatomical loss computation
    %      - Better gradient estimates
    %
    %   TRAINING RESULTS HISTORY:
    %   =========================
    %   Run #3 (v2.1, 5 epochs, 3 folds):
    %     - Sensitivity: 0.791, Specificity: 0.663 (good balance)
    %     - Accuracy: 0.726, F1-Score: 0.737
    %     - Observation: Balanced configuration worked well for short training
    %
    %   Run #4 (v2.1, 50 epochs, 5 folds):
    %     - Sensitivity: 0.551 ⚠️ (dropped from 0.791, -30.3%)
    %     - Specificity: 0.950 (increased from 0.663, +43.3%)
    %     - Accuracy: 0.754, F1-Score: 0.686 (decreased from 0.737)
    %     - OOD: Specificity collapsed to 0.292 (68.6% degradation)
    %     - CRITICAL OBSERVATION: Configuration became too conservative with long training
    %
    %   Run #5 (v2.2, 5 epochs, 3 folds):
    %     - Sensitivity: 0.899 ✅ (high, as intended)
    %     - Specificity: 0.255 ⚠️⚠️ (collapsed, too low)
    %     - OOD Specificity: 0.039 ⚠️⚠️⚠️ (near-zero, 89.91% degradation)
    %     - CRITICAL OBSERVATION: 3.03x bias was too aggressive
    %     - Root Cause: Model predicting PTB for almost all cases
    %
    %   KEY LEARNING:
    %   ============
    %   - v2.1 (2.23x bias): Good for 5 epochs, too conservative for 50 epochs
    %   - v2.2 (3.03x bias): Too aggressive, specificity collapsed
    %   - Solution: Find intermediate value between 2.23x and 3.03x
    %   - v2.3 uses 2.66x bias (intermediate value)
    %
    %   CURRENT CONFIGURATION (v2.3 - Optimized PTB Bias):
    %   ===================================================
    ...
    %   - focal_alpha: [0.32, 0.68] (reduced from [0.30, 0.70] in v2.2)
    %   - focal_gamma: 1.5 (reduced from 2.0)
    %   - class_weight_multiplier: 1.25x for PTB (reduced from 1.3x in v2.2)
    %   - Combined PTB bias: 2.66x (2.13 × 1.25, intermediate between v2.1 and v2.2)
    %
    %   EXPECTED PERFORMANCE (v2.3 Configuration):
    %   ==========================================
    %   - Sensitivity: 0.75-0.85 (maintained, balanced)
    %   - Specificity: 0.70-0.85 (recovered from collapse)
    %   - Accuracy: 0.75-0.80 (better overall balance)
    %   - F1-Score: 0.75-0.80 (improved balance)
    %
    %   RATIONALE FOR v2.3:
    %   ===================
    %   - v2.1 (2.23x): Too conservative for long training (sensitivity 0.551)
    %   - v2.2 (3.03x): Too aggressive (specificity 0.255 CV, 0.039 OOD)
    %   - v2.3 (2.66x): Intermediate value to balance sensitivity and specificity
    %   - Target: Maintain sensitivity while recovering specificity
    %
    %   RECOMMENDATIONS FOR FURTHER IMPROVEMENT:
    %   =======================================
    %   1. Monitor sensitivity/specificity balance during training
    %   2. Consider early stopping based on F1-score if sensitivity drops
    %   3. Experiment with adaptive class weighting during training
    %   4. Address OOD specificity collapse (test-time ROI extraction)
    %   5. Continue improving segmentation metrics (IoU/Dice)
...
% ============================================================================
% Run #1 (Baseline): Sensitivity 0.554 (too low) → Increased PTB bias
% Run #2 (Aggressive): Specificity 0.240 (collapsed) → Reduced PTB bias
% Run #3 (Balanced, 5 epochs): Sensitivity 0.791, Specificity 0.663 (good!)
% Run #4 (Balanced, 50 epochs): Sensitivity 0.551 ⚠️ (dropped!), Specificity 0.950
% Run #5 (Increased, 5 epochs): Sensitivity 0.899 ✅, Specificity 0.255 ⚠️⚠️ (collapsed)
% 
% KEY LEARNING: 
% - 2.23x bias: Good for 5 epochs, too conservative for 50 epochs
% - 3.03x bias: Too aggressive, specificity collapsed (0.255 CV, 0.039 OOD)
% - Solution: Use intermediate value 2.66x to balance sensitivity and specificity
% ============================================================================


% ============================================================================Grok RECOMMENDATIONS
%focal_alpha = [0.40, 0.60];          % was [0.32, 0.68] → reduces PTB weight
%class_weight_multiplier = 1.10;      % was 1.25x
%focal_gamma = 2.0;                   % increase back to 2.0 (you had reduced it)


config = struct();
% QUICK TEST MODE: Set to true for quick validation (3 folds, 5 epochs)
% Set to false for full training (5 folds, 50 epochs)
...
if QUICK_TEST
    ...
    config.k_folds = 5;
    config.numEpochs = 50;
    fprintf('FULL TRAINING MODE: 5 folds, 50 epochs\n');
end
config.patience = 10;
...
% Optimal hyperparameters (v2.3 - OPTIMIZED PTB BIAS)
    % UPDATED based on Run #5 analysis (specificity collapsed with 3.03x bias):
    % 1. Increased lambda_cam from 0.5 to 1.0 to improve segmentation quality
    % 2. Increased lambda_tversky from 2.5 to 5.0 to improve IoU/Dice scores
    % 3. REDUCED focal_alpha: [0.32, 0.68] (from [0.30, 0.70] in v2.2) to balance sensitivity/specificity
    %    - v2.2 [0.30, 0.70] with 1.3x multiplier = 3.03x bias was too aggressive
    %    - Result: Specificity collapsed (0.255 CV, 0.039 OOD) despite high sensitivity (0.899)
    %    - New [0.32, 0.68] provides 2.13x PTB weight (reduced from 2.33x)
    % 4. REDUCED class weight multiplier: 1.25x (from 1.3x in v2.2) to balance sensitivity/specificity
...
config.focal_alpha = [0.4, 0.6];  % [normal, PTB] - OPTIMIZED: Balance sensitivity and specificity
config.focal_gamma = 2;  % Reduced from 2.0 to reduce over-focusing
...
loss_config.lambda_anatomical = 15.0;  % NEW: Weight for anatomical guidance loss (increased significantly)
loss_config.anatomical_reward_weight = 0.75;  % UPDATED: Increased from 0.5 to 0.75 to better encourage lung focus
...
fprintf('  K-Folds: %d\n', config.k_folds);
fprintf('  λ_cam: %.4f (INCREASED from 0.5 to 1.0 to improve segmentation)\n', config.lambda_cam);
fprintf('  λ_tversky: %.4f (INCREASED from 2.5 to 5.0 to improve IoU/Dice)\n', config.lambda_tversky);
fprintf('  λ_anatomical: %.4f (IMPROVED: rewards inside + penalizes outside, SCALED ×1000)\n', loss_config.lambda_anatomical);
fprintf('  Anatomical reward weight: %.2f (INCREASED from 0.5 to 0.75)\n', loss_config.anatomical_reward_weight);
fprintf('  Focal alpha: [%.2f, %.2f] (OPTIMIZED: Balance sensitivity/specificity, 2.13x PTB weight)\n', config.focal_alpha(1), config.focal_alpha(2));
fprintf('  Class weight multiplier: 1.25x for PTB (reduced from 1.3x to balance sensitivity/specificity)\n');
fprintf('  Combined PTB bias: 2.66x (2.13 × 1.25, intermediate between v2.1 and v2.2)\n');
fprintf('  Focal gamma: %.2f (Reduced from 2.0 to reduce over-focusing)\n', config.focal_gamma);
fprintf('  NOTE: Anatomical loss is scaled by 1000 to match GradCAM magnitude\n');
fprintf('  Anatomical Guidance: %s\n', mat2str(loss_config.use_anatomical_guidance));
fprintf('  Samples per batch for anatomical loss: 32 (increased from 16)\n');
...
if strcmp(preprocessing_method_param, 'histmatch')
    ...
    % Try to find CXR directory (Target Domain) for reference
    % This aligns source (ROI) to target (CXR) distribution
    ...
    if ~useCXR
        ...
        fprintf('  WARNING: This may cause poor OOD performance due to domain shift.\n');
...
for fold = 1:config.k_folds
    ...
    % Class weights (OPTIMIZED: Balance sensitivity and specificity)
    classCounts = countcats(imdsTrain.Labels);
    baseWeights = sum(classCounts) ./ (numel(classCounts) * classCounts);
    % Apply optimized weighting to favor PTB class (1.25x, reduced from 1.3x in v2.2)
    % v2.2: 1.3x + focal_alpha [0.30, 0.70] = 3.03x bias was too aggressive (specificity collapsed)
    % v2.3: 1.25x + focal_alpha [0.32, 0.68] = 2.66x bias (intermediate value)
    if numel(classes) == 2
        classWeights = [baseWeights(1), baseWeights(2) * 1.25];  % Optimized PTB favor
    else
    ...
    % --- CHECKPOINTING ---
    % Save model after each fold to prevent data loss
    checkpointDir = fullfile('models', 'checkpoints');
    if ~exist(checkpointDir, 'dir')
        mkdir(checkpointDir);
    end
    
    checkpointFile = fullfile(checkpointDir, sprintf('checkpoint_%s_%s.mat', ...
        config.preprocessing_method, fold_name));
    
    % Save only necessary variables for this fold
...
while epoch < config.numEpochs
    ...
    while hasdata(mbqTrain)
        ...
        epochLoss = epochLoss + double(loss);
            ...
            % Log loss components periodically (every 10 batches)
    ...
    if mod(epoch, 3) == 0 || epoch == 1
        ...
        fprintf('  Val Loss: %.4f, Val Acc: %.3f, Val IoU: %.3f, Val Dice: %.3f\n', ...
            ...
            % Log loss component breakdown at validation
            if mod(epoch, 3) == 0 || epoch == 1
                % Compute a sample loss to show components
                try
                    [sample_loss, ~, ~, sample_components] = dlfeval(@compute_loss_with_config, ...
                        net, X, T, loss_config, classWeights, imdsTrain.Files, ...
                        preCAMs_train, preMasks_train, 8, classes, featureLayer, config.useGPU);
                    fprintf('  Loss breakdown: Cls=%.2f', double(sample_components.classification));
                    if loss_config.use_gradcam
                        fprintf(', CAM=%.2f (×%.1f=%.2f)', double(sample_components.gradcam), ...
                            loss_config.lambda_cam, double(sample_components.gradcam) * loss_config.lambda_cam);
                    end
                    if loss_config.use_tversky
                        fprintf(', Tversky=%.2f (×%.1f=%.2f)', double(sample_components.tversky), ...
                            loss_config.lambda_tversky, double(sample_components.tversky) * loss_config.lambda_tversky);
                    end
                    if loss_config.use_anatomical_guidance
                        fprintf(', Anatomical=%.2f (×%.1f=%.2f)', double(sample_components.anatomical), ...
                            loss_config.lambda_anatomical, double(sample_components.anatomical) * loss_config.lambda_anatomical);
                    end
                    fprintf('\n');
                catch
