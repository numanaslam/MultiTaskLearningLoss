...
%   OOD robustness improvements, and Test-Time Augmentation (TTA).
%
...
VALIDATION_MODE = true;  % Set to false for full training

config.patience = 25;
config.min_delta = 1e-2;
config.useGPU = canUseGPU;
config.batchSize = 14;
config.initialLearnRate = 0.0001;
config.decay = 0.0042;
...
config.lambda_cam = 10;            % Weight for GradCAM loss
config.lambda_tversky = 2.0;        % Weight for Tversky loss -> reduced from 7
...
loss_config.lambda_anatomical = 2; %reduced from 10
...
config.imageAugmenter = imageDataAugmenter( ...
    ...
    'RandRotation', [-30 30], ...
    'RandScale', [0.4 1.1]); % increased from [0.9 1.1]
...
fprintf('  Class weight multiplier: 1.2x for PTB\n');
...
%% === VALIDATION MODE: Use smaller dataset for quick testing ===
if VALIDATION_MODE
    fprintf('=== RUNNING IN VALIDATION MODE (Small Dataset) ===\n');
    
    % Option A: Random subset (fastest)
    subset_size = 150;  % Adjust based on your needs
    subset_idx = randperm(numel(imds.Files), subset_size);
    
    % Option B: Stratified subset (better for class balance)
    % classes = categories(imds.Labels);
    % subset_per_class = 75;
    % subset_idx = [];
    % for c = 1:numel(classes)
    %     class_idx = find(imds.Labels == classes{c});
    %     class_subset = class_idx(randperm(numel(class_idx), min(subset_per_class, numel(class_idx))));
    %     subset_idx = [subset_idx; class_subset];
    % end
    
    % Apply subset
    imds = subset(imds, subset_idx);
    
    % Also subset precomputed data if it exists
    ...
    % Adjust training config for small data
    config.k_folds = 2;           % Fewer folds
    config.numEpochs = 10;        % Fewer epochs
    config.patience = 3;          % Shorter patience
    config.batchSize = 8;         % Smaller batch if needed
    config.initialLearnRate = 0.0001; % Slightly lower LR
...
%% Out-of-Distribution (OOD) Evaluation with Ensemble and TTA
fprintf('\n=== OUT-OF-DISTRIBUTION EVALUATION (WITH ENSEMBLE + TTA) ===\n');
fprintf('Evaluating best model on Full CXR dataset (OOD)...\n');
fprintf('Using preprocessing ensemble and Test-Time Augmentation (Flip) for OOD robustness\n');
...
if exist(cxrDir, 'dir')
    ...
    % ID Evaluation (Pass false: No ROI extraction needed, but still use TTA)
    ...
    % === IMPROVEMENT: OOD evaluation with preprocessing ensemble and TTA ===
    fprintf('  Evaluating on Out-of-Distribution (Full CXR) data with preprocessing ensemble + TTA...\n');
    ...
    % ===============================================================
        ...
        %Calculate performance degradation using AUC (threshold-free)
    ...
    fprintf('    AUC gap:         %+.3f  (primary metric, threshold-free)\n', auc_gap);
    fprintf('    Accuracy gap:    %+.3f\n', acc_gap);
    fprintf('    Sensitivity gap: %+.3f  (at threshold %.3f / %.3f)\n', ...
        sens_gap, resultsID.optimal_threshold, resultsOOD.optimal_threshold);
    fprintf('    Specificity gap: %+.3f\n', spec_gap);

    if auc_gap < 0.05
        fprintf('    Status: Excellent generalization (AUC gap: %.3f)\n', auc_gap);
    elseif auc_gap < 0.10
        fprintf('    Status: Good generalization     (AUC gap: %.3f)\n', auc_gap);
    elseif auc_gap < 0.20
        fprintf('    Status: Moderate degradation    (AUC gap: %.3f)\n', auc_gap);
    else
        fprintf('    Status: Poor generalization     (AUC gap: %.3f) -- retrain\n', auc_gap);
    end
...
if exist('auc_gap', 'var')
    fprintf('  OOD AUC Gap (ID-OOD): %.3f\n', auc_gap);
end
...
fprintf('  ✓ Test-Time Augmentation (Horizontal Flip)\n');
...
for fold = 1:config.k_folds
    ...
    classWeights = baseWeights;   % pure inverse-frequency, no PTB multiplier
...
while epoch < config.numEpochs
    ...
    if mod(epoch, 3) == 0 || epoch == 1
        ...
        if valAcc > bestValAcc + config.min_delta
            ...
            bestNet = net;
            patienceCounter = 0;
            fprintf('  ✓ Balanced accuracy improved to %.3f\n', bestValAcc);
        else
            ...
            if ~isempty(realMask) && any(realMask(:))
                ...
                if isfield(loss_config, 'adaptive_scaling') && loss_config.adaptive_scaling
                    scale_factor = min(loss_config.max_anatomical_scale, ...
                        max(200, epoch * 4));  % Linear ramp: 100→1000 over 50 epochs
                else
...
% ============================================================================
% FUNCTION 1 — evaluate_dataset_with_preprocessing_ensemble  (replaces lines ~974-1113)
%
% KEY CHANGES vs original:
%   - Uses minibatchqueue + preprocessMiniBatchWithPreprocessing (same as training)
%     instead of apply_preprocessing_batch on raw float arrays.
%   - Collects probability scores first, then sweeps threshold to find the
%     operating point that maximises balanced accuracy (sens+spec)/2.
%   - Reports metrics at both default (0.5) and optimal threshold.
%   - PTB class index found by name lookup, not hardcoded.
% ============================================================================
function [results, predictions, uncertainty_stats] = evaluate_dataset_with_preprocessing_ensemble(...
    ...
    % ---- Build minibatch queue using SAME pipeline as training ----
    ...
    % ---- Find PTB class index by name (not hardcoded) ----
    ...
    % ---- Collect probability scores (no TTA in this unified path for clarity) ----
    Yprobs  = [];
    Ytrue   = imds.Labels(:);
    uncertainty_values = [];
    ...
    while hasdata(mbq)
        ...
        sc = predict(net, X);
        probs = extractdata(sc)';          % [batchSize x numClasses]
    ...
    % Trim to match label count (PartialMiniBatch='return' may add padding)
    Yprobs = Yprobs(1:numel(Ytrue));
    uncertainty_values = uncertainty_values(1:numel(Ytrue));

    % ---- Threshold sweep: find optimal balanced accuracy ----
    thresholds = 0.15:0.025:0.75;
    best_thresh  = 0.5;
    best_bal_acc = 0;
    thresh_results = zeros(numel(thresholds), 5);  % [thresh acc sens spec bal_acc]

    for ti = 1:numel(thresholds)
        t = thresholds(ti);
        preds_t = categorical(classes(1 + (Yprobs >= t)));  % 1=Normal(idx1), 2=PTB(idx2)
        ...
        if all(size(cm_t) == [2 2])
            ...
            acc_t = (TP_t + TN_t) / (TP_t + TN_t + FP_t + FN_t + eps);
            bal   = (s_t + sp_t) / 2;
            thresh_results(ti,:) = [t, acc_t, s_t, sp_t, bal];
            if bal > best_bal_acc
                best_bal_acc = bal;
                best_thresh  = t;
            end
    ...
    fprintf('    [eval] Optimal threshold: %.3f  (bal_acc=%.3f)\n', best_thresh, best_bal_acc);
    fprintf('    [eval] Default threshold: 0.500 (for reference)\n');
    ...
    unc_thresh = prctile(uncertainty_values, 80);
    uncertainty_stats.high_uncertainty_pct = mean(uncertainty_values > unc_thresh);
    uncertainty_stats.uncertainty_threshold = unc_thresh;
    ...
    results.balanced_accuracy = best_bal_acc;
    results.confusion_matrix  = cm;
    results.thresh_sweep      = thresh_results;  % full sweep for plotting
    ...
    predictions.threshold   = best_thresh;

end



%% Remaining helper functions (load_data_and_network, etc.)
%% Note: These functions are assumed to be defined in your workspace or file.
%% I include a few critical ones below to ensure the file is self-contained.
...
% ============================================================================
% FUNCTION 3 — apply_preprocessing_batch  (replaces lines ~1141-1156)
%
% KEY CHANGE: normalise float input to uint8 before histogram operations.
%             augmentedImageDatastore returns single/double [0,1] — the
%             original code silently cast these to uint8 zeros.
% ============================================================================
function imgs_processed = apply_preprocessing_batch(imgs, method, refHist)

    [H, W, C, B] = size(imgs);
    imgs_processed = zeros(H, W, C, B, 'uint8');

    for b = 1:B
        img = imgs(:,:,:,b);

        % --- Normalise float to uint8 if needed ---
        if ~isa(img, 'uint8')
            img_max = max(img(:));
            if img_max <= 1.0 && img_max > 0
                img = uint8(img .* 255);     % [0,1] float → [0,255] uint8
            elseif img_max > 1.0
                img = uint8(img ./ img_max .* 255);  % arbitrary float range
            else
                img = uint8(img);            % all zeros — nothing to do
            end
        end
        ...
        % --- Replicate to 3 channels to match VGG16 input ---
        if C == 3
...
function [imds, vggNet, precomputedGradCAM, precomputedMasks, classes] = load_data_and_network(forceRecalculate)
    ...
    modelPath = 'models/pretrained/vgg16_finetuned_on_roi.mat'; % Update path as needed
    if exist(modelPath, 'file')
        s = load(modelPath);
        vggNet = s.trainedNet;
    else
        error('Model file not found. Please check the path.');
    end
    roiDir = 'input/roi'; % Update path as needed
...
function plot_kfold_training_curves(training_histories, mean_metrics, std_metrics, outputDir)
    ...
    for f = 1:num_folds
        ...
        if isfield(hist, 'epoch_loss')
            plot(hist.epoch, hist.epoch_loss, '-', 'Color', colors(f,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Fold %d Train', f));
        end
        if isfield(hist, 'val_loss')
            val_epochs = hist.epoch(~isnan(hist.val_loss));
            val_loss = hist.val_loss(~isnan(hist.val_loss));
            if ~isempty(val_epochs), plot(val_epochs, val_loss, '--', 'Color', colors(f,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Fold %d Val', f)); end
        end
    end
    xlabel('Epoch'); ylabel('Loss'); title('Training and Validation Loss'); legend('Location', 'best'); grid on;
    ...
    for f = 1:num_folds
        ...
        if isfield(hist, 'val_accuracy')
            ...
            if ~isempty(val_epochs), plot(val_epochs, val_acc, '--', 'Color', colors(f,:), 'LineWidth', 1.5); end
        end
    end
    xlabel('Epoch'); ylabel('Accuracy'); title('Training and Validation Accuracy'); grid on;
    ...
    bar(metrics_values); hold on;
    errorbar(1:numel(metrics_values), metrics_values, [std_metrics.accuracy, std_metrics.precision, std_metrics.sensitivity, ...
                     std_metrics.specificity, std_metrics.f1_score, std_metrics.auc, ...
                     std_metrics.dice, std_metrics.iou], 'k.', 'LineWidth', 1.5);
    set(gca, 'XTickLabel', metrics_names); ylabel('Score'); title('Cross-Validation Metrics'); xtickangle(45); grid on; ylim([0, 1]);
...
% ============================================================================
% FUNCTION 2 — evaluateWithSegmentation  (replaces lines ~1286-1353)
%
% KEY CHANGES vs original:
%   - Collects probability scores first, sweeps threshold, reports at optimal.
%   - PTB class index found by name lookup.
%   - Segmentation loop unchanged.
% ============================================================================
...
function dist = improved_hausdorff_distance(pred, target)
    try
        ...
        pred_boundary = bwboundaries(pred, 'noholes'); target_boundary = bwboundaries(target, 'noholes');
        if isempty(pred_boundary) || isempty(target_boundary), dist = 0; return; end
        pred_areas = cellfun(@(x) size(x, 1), pred_boundary); target_areas = cellfun(@(x) size(x, 1), target_boundary);
        [~, pred_idx] = max(pred_areas); [~, target_idx] = max(target_areas);
        pred_pts = pred_boundary{pred_idx}; target_pts = target_boundary{target_idx};
        max_points = 100;
        if size(pred_pts, 1) > max_points, step = floor(size(pred_pts, 1) / max_points); if step > 1, pred_pts = pred_pts(1:step:end, :); else, pred_pts = pred_pts(1:max_points, :); end; end
        if size(target_pts, 1) > max_points, step = floor(size(target_pts, 1) / max_points); if step > 1, target_pts = target_pts(1:step:end, :); else, target_pts = target_pts(1:max_points, :); end; end
        dists_pred_to_target = zeros(size(pred_pts, 1), 1);
        for i = 1:size(pred_pts, 1), dists = sqrt(sum((target_pts - pred_pts(i, :)).^2, 2)); dists_pred_to_target(i) = min(dists); end
        dists_target_to_pred = zeros(size(target_pts, 1), 1);
        for i = 1:size(target_pts, 1), dists = sqrt(sum((pred_pts - target_pts(i, :)).^2, 2)); dists_target_to_pred(i) = min(dists); end
        h1 = max(dists_pred_to_target); h2 = max(dists_target_to_pred); dist = max(h1, h2);
        [h, w] = size(pred); dist = dist / sqrt(h^2 + w^2);
    catch, dist = 0; end
end
...
function valAcc = compute_validation_accuracy_simple(net, mbqVal, classes)
    ...
    % Sweep threshold, pick best balanced accuracy
    best_bal = 0;
    for t = 0.20:0.05:0.70
        preds_t = categorical(classes(1 + (Yprobs >= t)));
        cm_t = confusionmat(Ytrue_all, preds_t, 'Order', categorical(classes));
        if all(size(cm_t) == [2 2])
            s  = cm_t(ptbIdx,ptbIdx) / (cm_t(ptbIdx,ptbIdx) + cm_t(ptbIdx,3-ptbIdx) + eps);
            sp = cm_t(3-ptbIdx,3-ptbIdx) / (cm_t(3-ptbIdx,3-ptbIdx) + cm_t(3-ptbIdx,ptbIdx) + eps);
            bal = (s + sp) / 2;
            if bal > best_bal, best_bal = bal; end
        end
    end
    valAcc = best_bal;  % Early stopping now monitors balanced accuracy
end
...
function [val_acc, val_iou, val_dice] = evaluate_model_quick(net, imdsVal, classes, useGPU, ...
    ...
    % config is now required — callers must pass it through
    try
        ...
        [acc, ~, ~, ~, ~, ~, iou, dice, ~, ~, ~] = evaluateWithSegmentation(...
            net, imdsValSubset, classes, useGPU, ...
            gradCAM_subset, masks_subset, featureLayer, config);  % <-- config added
        val_acc = acc; val_iou = iou; val_dice = dice;
    catch ME
        fprintf('    [quick_eval] Warning: %s\n', ME.message);
        val_acc = 0.5; val_iou = 0.0; val_dice = 0.0;
    end
end

function focalLoss = compute_focal_loss(Y, T, classWeights, alpha, gamma)
    epsVal = 1e-7; Y = min(max(Y, epsVal), 1-epsVal);
    if isscalar(alpha), alpha_vec = alpha * reshape(classWeights, [], 1);
    else, alpha_vec = reshape(alpha(:), [], 1) .* reshape(classWeights, [], 1); end
...
function tverskyCoef = tversky_coefficient_dlarray(pred, target, alpha, beta)
    if isa(pred, 'dlarray')
        ...
        pred = logical(gather(pred)); target = logical(gather(target));
        intersection = sum(pred(:) & target(:)); fp = sum(pred(:) & ~target(:)); fn = sum(~pred(:) & target(:));
        tverskyCoef = intersection / (intersection + alpha * fp + beta * fn + eps);
    end
end
...
function diceCoef = dice_coefficient_dlarray(pred, target)
    if isa(pred, 'dlarray')
        target_dl = dlarray(single(target), 'SSC');
        intersection = sum(pred(:) .* target_dl(:), 'all');
        pred_sum = sum(pred(:), 'all'); target_sum = sum(target_dl(:), 'all');
